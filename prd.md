...
- **RF-M1-11** *(novo — pedido do usuário em 2026-09-03)* — **Romanização (pinyin) do resultado em chinês.** Quando o idioma de destino é `zh`, o painel de destino exibe, **acima** do hanzi e em estilo secundário, a leitura em pinyin com marcas de tom. A linha é omitida — não exibida vazia — quando o destino não é `zh` ou quando o resultado não contém caractere Han (número, sigla). O pinyin é apoio de leitura: **não** entra em copiar, compartilhar ou favoritar, e **não** é falado pelo TTS (o motor lê o hanzi). Falha na romanização não é erro de tradução: a linha some e o hanzi permanece. Conversão 100% local, por dicionário de EXPRESSÕES — o que garante os polifônicos (长大 ⇒ `zhǎng dà`, 银行 ⇒ `yín háng`, 了 ⇒ `le`). Anatomia em `docs/design_system.md` §5.13.
